    ...
    <>
      <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 w-full max-w-lg">
        ...
        <div className="relative w-full sm:w-auto flex-grow">
          ...
          {/* 최근 검색어 목록 */}
          {showRecentSearches && (
            <RecentPlaceSearchLists
              recentSearches={recentSearches}
              onSearchSelect={handleSelectSearch}
          ...
          {/* 자동완성 목록 */}
          {showAutoComplete && (
            <ul className="absolute left-0 w-full bg-white border border-gray-300 mt-1 max-h-40 sm:max-h-60 overflow-y-auto shadow-lg p-2 rounded-md">
              {suggestedUserIds.map((completedId, index) => (
                <li
                  key={index}
                  className="p-2 cursor-pointer hover:bg-gray-200"
                  onClick={() => handleSelectSearch(completedId)}
                >
                  {completedId}
                </li>
              ))}
        ...
        <Link
          ...
          className="bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 transition-transform transform hover:scale-105 w-full sm:w-auto"
